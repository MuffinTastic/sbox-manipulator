...
public class PropertySheet : Widget
{
	...
	internal Widget AddProperty( object target, string targetName )
	{
		...
		return AddProperty( prop, ref target );
	}

	internal void AddStaticProperty( System.Type target, string targetName )
	{
		var prop = target.GetProperties( BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic ).FirstOrDefault( x => x.Name == targetName );
		if ( prop == null )
		{
			Log.Warning( $"Static property {targetName} not found" );
			return;
		}

		object nullObj = null;
		var addedRow = AddProperty( prop, ref nullObj );
	}
	...
	{
		if ( !target.TryGetValue( targetName, out var val ) || val == null )
		{
			target[targetName] = defaultValue;
		}
	...
	Widget AddProperty( PropertyInfo prop, ref object target )
	{
		...
		var w = CanEditAttribute.CreateEditorFor( prop, ref target );
		if ( w != null )
		{
			var row = new PropertyRow( this );
			row.SetLabel( di );
			...
			if ( prop.GetCustomAttributes<ValidationAttribute>().Any() )
			{
				var wb = new PropertyRowError();
				AddRow( wb );
				var t = target;

				void CheckForErrors()
				{
					var succ = prop.CheckValidationAttributes( t, out string[] errors, di.Name );

					row.Errors = !succ;
					wb.Visible = !succ;
					wb.Label.Text = string.Join( "\n", errors );
				}

				w.OnChildValuesChanged += ( Widget src ) => CheckForErrors();
				CheckForErrors();
			}
			...
			{
				w.ReadOnly = true;
				w.Enabled = false; // TODO: ReadOnly barely does anything yet.
		...
		bool childPropertySheet = /*prop.PropertyType.IsClass ||*/ prop.PropertyType.IsValueType;
